...
namespace CarsAndBids.API.Filters;
...
public class ValidationFilter : IAsyncActionFilter
{
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        foreach (var (paramName, argument) in context.ActionArguments)
        {
            ...
            // Ручна валідація простих типів (int id)
            if (argument is int intValue && intValue <= 0 && paramName.ToLower().Contains("id"))
            {
            ...
            // Валідація моделей через FluentValidation
